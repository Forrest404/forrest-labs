...
export async function GET(
  ...
  if (cluster.status === 'confirmed' || cluster.status === 'auto_confirmed') {
    return new Response(
      buildHtml('#450a0a', '#fca5a5', '✗ Already confirmed', 'This cluster was already confirmed and cannot be rejected.'),
  ...
  if (updateError) {
    console.error('[reject] Failed to reject cluster:', updateError.message)
    return new Response(
  ...
  if (ntfyChannel) {
    await fetch(`https://ntfy.sh/${ntfyChannel}`, {
      ...
      console.error('[reject] ntfy notification failed:', err)
    })
  }

  return new Response(
    buildHtml('#450a0a', '#fca5a5', '✗ Rejected', 'Cluster rejected and removed from the map.'),
    { status: 200, headers: { 'Content-Type': 'text/html' } },
  )
}
