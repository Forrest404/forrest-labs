...
export async function GET(request: NextRequest) {
  ...
  // notif_setup_done is additive — fall back to a select without it pre-migration.
  const base = 'full_name, email, phone, role, language, notif_push, notif_sms, quiet_start, quiet_end, off_duty, password_hash, pin_hash, totp_enabled'
  let res: any = await supabase.from('ngo_users').select(`${base}, notif_setup_done`).eq('id', session!.userId).maybeSingle()
  if (res.error && (res.error.code === 'PGRST204' || res.error.code === '42703')) {
    res = await supabase.from('ngo_users').select(base).eq('id', session!.userId).maybeSingle()
  }
  ...
  return NextResponse.json({
    account: {
      ...
      notif_setup_done: (data as any).notif_setup_done ?? false,
...
export async function PATCH(request: NextRequest) {
  ...
  if (body.notif_setup_done !== undefined) update.notif_setup_done = !!body.notif_setup_done
