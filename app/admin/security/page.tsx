...
// Admin 2FA (TOTP) enrolment. Opt-in: applying the migration does NOT lock anyone out —
// 2FA only takes effect once enabled here. Setup → scan QR → verify a code → enabled, with
...
export default function AdminSecurityPage() {
  ...
  async function act(action: string, extra: Record<string, unknown> = {}) {
    ...
    try {
      const r = await fetch('/api/admin/2fa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, ...extra }) })
  ...
  const enable = async () => {
    const d = await act('enable', { code: code.trim() })
    if (d) { setRecoveryCodes(d.recovery_codes ?? []); setSetup(null); setCode(''); setMsg('Two-factor authentication is on.'); load() }
  }
  const disable = async () => {
    if (!window.confirm('Disable two-factor authentication for the admin panel?')) return
    const d = await act('disable', { code: code.trim() })
    if (d) { setCode(''); setMsg('Two-factor authentication disabled.'); load() }
  }

  return (
    <div style={{ maxWidth: 560, padding: 24, color: '#e6edf3', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 20, fontWeight: 600, margin: '0 0 4px' }}>Admin security</h1>
      <p style={{ fontSize: 13, color: '#8b949e', margin: '0 0 20px' }}>Two-factor authentication for the admin panel (authenticator app).</p>
      ...
      {recoveryCodes && (
        <div style={card}>
          ...
          <div style={{ fontSize: 12, color: '#8b949e', marginBottom: 12 }}>Each works once if you lose your authenticator. Store them somewhere safe — they won’t be shown again.</div>
      ...
      {enabled === false && !setup && (
        <div style={card}>
          <div style={{ fontSize: 14, marginBottom: 12 }}>2FA is <b style={{ color: '#d29922' }}>off</b>. Strongly recommended for the admin account.</div>
...
const field: React.CSSProperties = { width: '100%', maxWidth: 260, height: 42, padding: '0 12px', boxSizing: 'border-box', background: '#0d1117', border: '1px solid #21262d', borderRadius: 6, color: '#e6edf3', fontSize: 16, fontFamily: 'system-ui', outline: 'none', letterSpacing: '0.1em' }
const primaryBtn: React.CSSProperties = { height: 40, padding: '0 18px', background: '#238636', border: '1px solid #2ea043', color: '#fff', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'system-ui' }
const dangerBtn: React.CSSProperties = { height: 38, padding: '0 16px', background: 'rgba(248,81,73,0.1)', border: '1px solid rgba(248,81,73,0.4)', color: '#f85149', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'system-ui' }
