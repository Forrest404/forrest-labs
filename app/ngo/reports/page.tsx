...
export default function NgoReportsPage() {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) { setNote({ kind: 'error', text: d?.error ?? 'Generation failed.' }); return }
      setTitle('')
      await loadList()
      // Open the freshly generated report so the draft shows immediately.
      if (d.report?.id) await view(d.report.id)
      setNote(d.ai_error ? { kind: 'info', text: d.ai_error } : { kind: 'info', text: 'Situation report generated.' })
    } catch {
      setNote({ kind: 'error', text: 'Generation failed — check your connection and try again.' })
  ...
  const view = useCallback(async (id: string) => {
    ...
    try {
      ...
      else setNote({ kind: 'error', text: 'Could not open that report.' })
    } catch { setNote({ kind: 'error', text: 'Could not open that report.' }) }
  ...
  if (forbidden) {
    return (
      <div style={wrap}>
        ...
        <div style={emptyBox}>Reports are available to team leaders and organisation admins.</div>
      </div>
    )
  }
  ...
  return (
    <div style={wrap}>
      <h1 style={h1}>Reports</h1>
      <p style={sub}>Generate situation reports from your incidents and dispatches, and export your data.</p>
      ...
      {/* (a) Generate */}
      <section style={card}>
        <div style={cardTitle}>Generate situation report</div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
          <label style={field}><span style={lbl}>From</span><input type="date" value={start} max={end} onChange={(e) => setStart(e.target.value)} style={input} /></label>
          <label style={field}><span style={lbl}>To</span><input type="date" value={end} min={start} max={todayISO()} onChange={(e) => setEnd(e.target.value)} style={input} /></label>
        </div>
        <label style={{ ...field, width: '100%' }}><span style={lbl}>Title (optional)</span>
          <input type="text" value={title} placeholder="Auto-titled from the date range" onChange={(e) => setTitle(e.target.value)} style={input} />
        </label>
      ...
      {open && (
        <section style={{ ...card, borderColor: '#3fb950' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
            ...
            <button type="button" onClick={() => setOpen(null)} style={ghostBtn}>Close</button>
          </div>
          ...
          <div id="report-draft" style={{ fontSize: 14, lineHeight: 1.6, color: '#e6edf3', whiteSpace: 'pre-wrap', background: '#0d1117', border: '1px solid #21262d', borderRadius: 8, padding: 14 }}>
            {open.draft ?? 'No AI narrative was generated for this report (the underlying data is still saved and exportable).'}
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
            <button type="button" onClick={() => window.open(`/api/ngo/reports/${open.id}/export?format=doc`, '_blank')} style={secondaryBtn}>Export Word</button>
            <button type="button" onClick={() => printDraft(open)} style={secondaryBtn}>Print / Save as PDF</button>
          </div>
        </section>
      )}
      ...
      {/* (b) Saved reports */}
      <section style={card}>
        <div style={cardTitle}>Saved reports</div>
        {!listLoaded && <div style={muted}>Loading…</div>}
        {listError && <div style={errBox}>Couldn’t load reports. <button type="button" onClick={loadList} style={retryBtn}>Retry</button></div>}
        {listLoaded && !listError && reports.length === 0 && <div style={emptyBox}>No reports yet — generate one above.</div>}
            ...
            <div key={rep.id} style={row}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: '#8b949e', marginTop: 2 }}>
                  {String(rep.period_start).slice(0, 10)} → {String(rep.period_end).slice(0, 10)} · {new Date(rep.created_at).toLocaleDateString()}{!rep.has_draft ? ' · data only' : ''}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                <button type="button" disabled={busyId === rep.id} onClick={() => view(rep.id)} style={miniBtn}>View</button>
                <button type="button" onClick={() => window.open(`/api/ngo/reports/${rep.id}/export?format=doc`, '_blank')} style={miniBtn}>Word</button>
                <button type="button" disabled={busyId === rep.id} onClick={() => del(rep)} style={{ ...miniBtn, color: '#f85149', borderColor: 'rgba(248,81,73,0.4)' }}>Delete</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* (c) Export data */}
      <section style={card}>
        <div style={cardTitle}>Export data</div>
        <p style={{ fontSize: 12, color: '#8b949e', margin: '0 0 10px' }}>Download your incidents and dispatches for the selected range above.</p>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button type="button" onClick={() => exportData('csv')} style={secondaryBtn}>Download CSV</button>
          <button type="button" onClick={() => exportData('geojson')} style={secondaryBtn}>Download GeoJSON</button>
        </div>
      </section>
    </div>
  )
