...
interface User { id: string; full_name: string | null; email: string; phone: string | null; role: string; status: string }
type AddForm = { full_name: string; email: string; phone: string; role: string; password: string; pin: string }
type EditForm = { id: string; full_name: string; phone: string; role: string; status: string; password: string; pin: string }
...
export default function NgoUsersPage() {
  ...
  async function createUser() {
    ...
    try {
      ...
      if (res.ok) { setAdd(null); setMsg('User added.'); await load() }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (res.ok) { setEdit(null); setMsg('User updated.'); await load() }
  ...
  return (
    <div style={wrap}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
        ...
        <button type="button" onClick={() => setAdd({ full_name: '', email: '', phone: '', role: 'team_leader', password: '', pin: '' })} style={primaryBtn}>+ Add user</button>
      </div>
          ...
          <div key={u.id} style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                <button type="button" onClick={() => setEdit({ id: u.id, full_name: u.full_name ?? '', phone: u.phone ?? '', role: u.role, status: u.status, password: '', pin: '' })} style={miniBtn}>Edit</button>
      ...
      {add && (
        <Modal title="Add user" onClose={() => setAdd(null)}>
          ...
          {add.role === 'field_coordinator'
            ? <L label="PIN (4–6 digits)"><input style={field} inputMode="numeric" value={add.pin} onChange={(e) => setAdd({ ...add, pin: e.target.value })} /></L>
          ...
          <button type="button" onClick={createUser} disabled={busy} style={{ ...primaryBtn, marginTop: 14, opacity: busy ? 0.6 : 1 }}>{busy ? 'Adding…' : 'Add user'}</button>
        </Modal>
      )}
      ...
      {edit && (
        <Modal title="Edit user" onClose={() => setEdit(null)}>
          ...
          {edit.role === 'field_coordinator'
            ? <L label="Reset PIN (optional, 4–6 digits)"><input style={field} inputMode="numeric" value={edit.pin} onChange={(e) => setEdit({ ...edit, pin: e.target.value })} /></L>
          ...
          <button type="button" onClick={saveEdit} disabled={busy} style={{ ...primaryBtn, marginTop: 14, opacity: busy ? 0.6 : 1 }}>{busy ? 'Saving…' : 'Save'}</button>
        </Modal>
      )}
    </div>
  )
